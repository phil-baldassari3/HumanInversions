...
def compute_SNP_to_SNV_ratio(SNPbedgraph, SNVbedgraph):
    ...
    merged_df["ALL_VARIANTS"] = merged_df["SNP_count"] + merged_df["SNV_count"]

    #output new file
    merged_df = merged_df[["CHROM", "START", "END", "ALL_VARIANTS"]]
    merged_df.to_csv(SNPbedgraph.replace("SNP_count", "all_biallelic_variant_count"), sep="\t", index=False, header=False)
